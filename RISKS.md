...
| # | Failure | Likelihood | Impact | Detection | Mitigation | Residual risk |
|---|---|---|---|---|---|---|
| T1 | False confidence: scoped selection (W3) misses the suite that would have caught a regression | M | M (advisory in POC) | Phase 11 eval: replay historical PRs, compare selected vs. suites that actually failed; recall metric | Selection = deterministic superset (reverse import closure ∪ map ∪ LLM additions); unmappable → full suite; POC never replaces required full CI | M — recall <100% is inherent; that's why it stays advisory until measured |
...
| # | Failure | Likelihood | Impact | Detection | Mitigation | Residual risk |
|---|---|---|---|---|---|---|
| G7 | **Duplicate work: assistant merges a dependency bump that a human PR is concurrently landing** (observed: #16768 cel-go, closed by a maintainer because human PR #16782 shipped the same bump as a CVE fix) | M (measured 1/50) | L-M (churn, conflicts, wasted review) | Eval replay found it; in production, a merge conflict or duplicate commit | **Not yet implemented:** add `no_competing_pr` policy rule — deny when another open PR modifies the same dependency/module path. Roadmap item. | M until the rule ships |
