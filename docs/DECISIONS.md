...
- **Trigger mechanism for the POC: polling sweep + manual CLI trigger**, not live webhooks. `poll` mode lists open Dependabot PRs / new issues since last run; `run --pr N` triggers one run for the demo. Webhook ingestion is an adapter swap on the same run entrypoint, listed as roadmap.
...
- **Pure event-driven webhook service.** Right for production, wrong first build for a POC: needs public endpoint, webhook secret handling, delivery dedupe (RISKS G3) — all infrastructure, no thesis. Polling gives identical downstream behavior on the same entrypoint.
...
**Consequences.** (a) The POC binary is a CLI (`assistant poll`, `assistant run --pr N`, `assistant serve` reserved for roadmap); (b) horizontal scaling and webhook latency are explicitly out of POC scope; (c) every run is independently replayable from its audit record, which Phase 11's eval harness reuses directly.
